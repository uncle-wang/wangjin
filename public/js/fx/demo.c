...
// 是否可以以AAB模式开仓
void _aabAvailable(string& group[], double spreadCost, double digitsNumb) {
	...
	double bb = (priceC -  priceA * priceB) * digitsNumb;
	if (bb - spreadCost >= 0) {
		Print("扭曲：", bb);
...
// 是否可以以BBA模式开仓
void _bbaAvailable(string& group[], double spreadCost, double digitsNumb) {
	...
	double bb = (priceA * priceB - priceC) * digitsNumb;
	if (bb - spreadCost >= 0) {
		Print("扭曲：", bb);
		Print(priceA, " ", priceB, " ", priceC);
		Print(group[0], group[1], group[2]);
...
void eaPriceAvailable() {
	...
	for (int i = 0; i < ArrayRange(symbolGroups, 0); i ++) {
		...
		_aabAvailable(group, spreadCost, digitsNumb);
		_bbaAvailable(group, spreadCost, digitsNumb);
	}
	return;
}
